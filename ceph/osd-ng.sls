...
zap_disk_{{ data_path }}:
  cmd.run:
    - name: 'ceph-volume lvm zap {{ data_path }}'
...
{% for removed_osd in ceph.osds.removed %}
    {% set osd_id = salt['cmd.run']("ceph-volume lvm list | grep '" ~ removed_osd ~ "1 ceph' | awk -F'[.,]' '{print $5}'") %}

osd_out_{{ removed_osd }}:
  cmd.run:
    - name: 'ceph osd out {{ osd_id }}'
    - unless: "ceph-volume lvm list | grep -E ' *{{ removed_osd }}1? .*ceph data, unprepared'"
